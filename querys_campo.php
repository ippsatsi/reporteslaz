...
, MGA.DIR_ESTADO_VALIDEZ AS 'S10|HISTORICO'
...
, SUM(CASE WHEN CONVERT(DATE,RNK.DIR_FECHA_REGISTRO,105)=CONVERT(DATE,DATEADD(DD,-1,GETDATE()),105) THEN 1 END) AS DIA_8
FROM (
SELECT
BUSQ.DIR_CODIGO
, BUSQ.GES_TOTALES
...
function reporte_3() {
  ...
  $query = "
SELECT
 USU.USU_LOGIN AS 'S10|USUARIO'
, BUSQ.GES_TOTALES AS 'N12|GESTIONES TOTALES'
, BUSQ.GESTION_POSTERIOR AS 'N12|GESTIONES DESPUES'
, BUSQ.GESTION_ANTERIOR AS 'N12|GESTIONES ANTES'
--, BUSQ.DIR_FECHA_REGISTRO AS 'D18|FECHA REGISTRO1'
, CONVERT(VARCHAR, BUSQ.DIR_FECHA_REGISTRO, 103) AS 'S14|DIA REGISTRO'
, CONVERT(VARCHAR, BUSQ.DIR_FECHA_REGISTRO, 108) AS 'S14|HORA REGISTRO'
, CLI.CLI_DOCUMENTO_IDENTIDAD AS 'S14|DOCUMENTO'
, CUE.CUE_NROCUENTA AS 'S22|CUENTA'
, SCA.SCA_DESCRIPCION AS 'S14|CARTERA'
, BUSQ.DIR_ESTADO_VALIDEZ AS 'S08|ESTADO'
, UBI.UBI_DEPARTAMENTO AS 'S16|DEPARTAMENTO'
, UBI.UBI_PROVINCIA AS 'S18|PROVINCIA'
, UBI.UBI_DISTRITO AS 'S30|DISTRITO'
, REPLACE(BUSQ.DIR_DIRECCION, CHAR(10), '') AS 'S50|DIRECCION'
FROM
COBRANZA.GCC_CLIENTE CLI
INNER JOIN (
SELECT
...
DIR.DIR_FECHA_REGISTRO BETWEEN CONVERT(DATETIME,DATEADD(DD,-7,CONVERT(DATE,GETDATE(),105)),105)
 AND CONVERT(DATETIME,DATEADD(DD,1,CONVERT(DATE,GETDATE(),105)),105)
 GROUP BY 
 DIR.DIR_CODIGO, DIR.CLI_CODIGO, DIR.DIR_FECHA_REGISTRO
 , DIR.DIR_USUARIO_REGISTRO, DIR.UBI_CODIGO, DIR.DIR_ESTADO_VALIDEZ
 , DIR.DIR_DIRECCION
 ) BUSQ ON BUSQ.CLI_CODIGO=CLI.CLI_CODIGO
 INNER JOIN COBRANZA.GCC_USUARIO USU ON USU.USU_CODIGO=BUSQ.DIR_USUARIO_REGISTRO
 INNER JOIN COBRANZA.GCC_UBIGEO UBI ON UBI.UBI_CODIGO=BUSQ.UBI_CODIGO
