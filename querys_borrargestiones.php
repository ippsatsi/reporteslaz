...
CASE
   ...
   ELSE CONCAT('<input type=\"checkbox\" name=\"check_list[]\" value=\"',GES.GES_CODIGO,'\">') END AS CODIGO
, CUE.CUE_NROCUENTA
...
ORDER BY 1 DESC;";
  ...
  $result_query = sqlsrv_query( $conn, $query, PARAMS_MSSQL_QUERY, OPTIONS_MSSQL_QUERY );
    if (!$result_query) {
    throw new Exception('No se pudo completar la consulta de gestiones',4);
  }
  if(sqlsrv_num_rows($result_query)>0){
    $array_query_result= array();
    while( $row = sqlsrv_fetch_array($result_query, SQLSRV_FETCH_NUMERIC) ) {
...
function borrar_gestiones($str_gestiones) {
  $conn = conectar_mssql();
  $query = "
  UPDATE GES
  SET
  GES.GES_ESTADO_REGISTRO='I'
  , GES.TXMOTIVOBAJA='IP:".$_SERVER['REMOTE_ADDR']."'
  , FEBAJA=GETDATE()
  FROM
