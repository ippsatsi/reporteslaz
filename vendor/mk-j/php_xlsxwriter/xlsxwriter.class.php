...
class XLSXWriter
{
	...
	public function writeSheetHeaderFormated($sheet_name, $cabecera, $style)
	{
	    $col_options['widths'] = array();
        $header = array();
        foreach ($cabecera as &$value) {//N10|documento
        $campo = explode('|', $value); //$campo[0] = N10 , $campo[1]=documento
        $value = $campo[1];//$value = documento
        $datos_columna = str_split($campo[0]);
        $tipo_columna = $datos_columna[0];//$tipo_columna=N
        $width_columna = intval($datos_columna[1].$datos_columna[2]);//$width_columna=10
        $col_options['widths'][] = $width_columna; //llenamos col_options
        switch ($tipo_columna) {
            case 'N':
                $header[] = 'number';
                break;
      ...
      $cabecera = array_combine($cabecera,$header ); // combina los encabezados con los tipos de columna

      $this->writeSheetHeader($sheet_name, $cabecera, $col_options , $style );
	}

	public function writeSheetHeader($sheet_name, array $header_types, $col_options = null, $row_options=null)
	{
    ...
    $style = &$col_options;
		...
		if (!$suppress_row)
		{
			...
			$sheet->file_writer->write('<row collapsed="'.($collapsed).'" customFormat="false" customHeight="'.($customHt).'" hidden="'.($hidden).'" ht="'.($ht).'" outlineLevel="0" r="' . (1) . '">');
		//	$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="false" hidden="false" ht="12.1" outlineLevel="0" r="' . (1) . '">'); //antiguo comando
